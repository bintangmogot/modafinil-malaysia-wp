    ...
    <div class="container-site relative flex h-[68px] items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 ml-auto lg:ml-0">
            ...
            <?php if (shortcode_exists('xoo_wsc_cart')): ?>
                <div class="header-side-cart-trigger text-foreground/70 hover:text-primary transition-colors">
                    <?= do_shortcode('[xoo_wsc_cart]') ?>
                </div>
            <?php else: ?>
                <a href="<?= wc_get_cart_url() ?>" aria-label="Kedai / Shop" class="rounded-md p-2 text-foreground/70 transition-colors hover:text-primary">
