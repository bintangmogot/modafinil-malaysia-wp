...
if (is_home() && $has_modules) {
    // If it's the blog page and has ACF modules, render them (at least the hero)
    while (have_rows('modules', $blog_page_id)) {
        the_row();
        $layout = get_row_layout();
        if ($layout === 'page_hero') {
            get_template_part('modules/content', $layout);
            break; // Just render the hero
        }
    }
} else {
    // Hardcoded fallback Hero (Matches React app)
...
<section class="section-y bg-background min-h-[50vh]">
    ...
    <div class="container-site mt-12 flex justify-center">
        ...
        // Replace WordPress classes with Tailwind
