<?php
/**
 * Template for single City CPT posts
 */
get_header();
...
$delivery_days = get_field('delivery_days') ?: '';
$demographic_en = get_field('demographic_en') ?: '';
$demographic_ms = get_field('demographic_ms') ?: '';
$industry_en = get_field('industry_en') ?: '';
$industry_ms = get_field('industry_ms') ?: '';
$hero_desc_en = get_field('hero_desc_en') ?: '';
$hero_desc_ms = get_field('hero_desc_ms') ?: '';
$desc_en = get_field('desc_en') ?: '';
$desc_ms = get_field('desc_ms') ?: '';
...
$city_faqs = [
    ...
    [
        ...
        'a_ms' => "Ya — setiap pesanan ke {$city_name} dihantar dalam pembungkusan biasa tanpa jenama, nama produk, atau maklumat yang mengenal pasti kandungannya."
    ],
    [
        'q_en' => "Do many people in {$city_name} use Modafinil?",
        'q_ms' => "Ramai orang di {$city_name} menggunakan Modafinil?",
        'a_en' => "Yes, {$city_name} is one of our most active delivery areas. " . ($demographic_en ? ucfirst($demographic_en) : 'Professionals') . " and " . ($industry_en ? 'people in ' . $industry_en : 'many sectors') . " regularly order from us.",
        'a_ms' => "Ya, {$city_name} adalah salah satu kawasan penghantaran kami yang paling aktif. " . ($demographic_ms ?: 'Profesional') . " dan mereka dalam " . ($industry_ms ?: 'pelbagai sektor') . " secara berkala memesan dari kami."
    ],
    [
        'q_en' => "What happens if my order to {$region} doesn't arrive?",
        'q_ms' => "Apa yang berlaku jika pesanan saya ke {$region} tidak tiba?",
        'a_en' => "In the rare event your order is lost or delayed beyond the expected window, we offer a free reshipment. Contact us via WhatsApp for the fastest resolution.",
        'a_ms' => "Dalam kes yang jarang berlaku di mana pesanan anda hilang atau lewat, kami menawarkan penghantaran semula percuma. Hubungi kami via WhatsApp untuk penyelesaian paling cepat."
    ]
];
?>
        ...
        <div class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <div class="shrink-0">
                <div class="min-w-[160px] rounded-2xl bg-white/15 p-5 text-center backdrop-blur border border-white/10 shadow-lg">
                    ...
                    <p class="text-xs font-medium uppercase tracking-wider text-emerald-200"><?= modmy_t("Delivery Days", "Hari Penghantaran") ?></p>
    ...
    <div class="container-custom max-w-4xl">
        ...
        <div class="mb-10 grid gap-5 md:grid-cols-2">
            ...
            foreach($features as $idx => $feat):
                ...
                $feat_text = modmy_t($feat['feat_en'] ?? '', $feat['feat_ms'] ?? '');
            ...
            <div class="flex items-start gap-3 rounded-xl border border-emerald-100 bg-emerald-50/50 p-4 shadow-sm">
                <?= $icon ?>
                <span class="text-sm font-semibold text-slate-800 leading-snug"><?= esc_html($feat_text) ?></span>
            </div>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>

        <?php if ($desc_en || $desc_ms): ?>
    ...
    <div class="container-custom max-w-5xl">
        ...
        <div class="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
            ...
            <div class="group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-stone-200 transition-all hover:-translate-y-1 hover:shadow-xl hover:ring-emerald-500">
                <a href="<?= get_permalink($product->ID) ?>" class="absolute inset-0 z-10"><span class="sr-only">View <?= esc_html($product->post_title) ?></span></a>
                ...
                <div class="flex flex-1 flex-col p-4">
                    <h3 class="font-heading text-lg font-black text-slate-900"><?= esc_html($product->post_title) ?></h3>
    ...
    <div class="container-custom py-8">
        <div class="grid grid-cols-2 gap-6 md:grid-cols-4">
            ...
            <div class="flex flex-col items-center gap-3 rounded-xl px-2 text-center">
                ...
                <p class="text-xs font-bold uppercase leading-tight tracking-wider text-slate-700"><?= modmy_t($b['en'], $b['ms']) ?></p>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
        ...
        <div class="mb-10 text-center">
            ...
            <div class="mt-4 flex items-center justify-center gap-2">
                <div class="flex text-amber-400">
                    <?php for($i=0; $i<5; $i++): ?><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg><?php endfor; ?>
                </div>
                <span class="text-sm font-semibold text-slate-600"><?= modmy_t("4.5 out of 5 (" . count($reviews) . " reviews)", "4.5 dari 5 (" . count($reviews) . " ulasan)") ?></span>
            </div>
        </div>
        <div class="space-y-5">
            ...
            <div class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div class="flex gap-4">
                        <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-100 font-heading text-lg font-black text-emerald-700"><?= esc_html(substr($review['author'], 0, 1)) ?></span>
                        <div>
...
<section class="section-padding bg-white">
    <div class="container-custom max-w-3xl">
        ...
        <div class="space-y-4">
            ...
            <details class="group rounded-2xl border border-stone-200 bg-white p-6 shadow-sm open:bg-emerald-50/30 open:border-emerald-200 cursor-pointer" <?= $i === 0 ? 'open' : '' ?>>
                ...
                <p class="mt-4 text-sm leading-relaxed text-slate-600 pr-8"><?= modmy_t($faq['a_en'], $faq['a_ms']) ?></p>
            </details>
            <?php endforeach; ?>
        </div>
    </div>
